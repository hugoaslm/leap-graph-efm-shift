...
from _paths import find_neural_lam
...
def select_eval_indices(
    ...
    day_strs = np.datetime_as_string(base_times, unit="D")
    ...
    selected = []
    for month_key in sorted({s[:7] for s in by_day}):
...
def compute_all_metrics(
    ...
    for lead_idx in range(n_steps):
        ...
        p_lead = pred[:, :, lead_idx]
        t_lead = target[:, lead_idx]

        for var_idx, vname in enumerate(var_names):
            p_v = p_lead[..., var_idx:var_idx+1]
            t_v = t_lead[..., var_idx:var_idx+1]

            ens_mean = p_v.mean(dim=1)
...
def compute_persistence_rmse(
    targets: np.ndarray,
    init_states_for_persistence: np.ndarray,
    grid_weights: torch.Tensor,
    var_names: list,
    lead_hours: list,
    mask: torch.Tensor | None = None,
