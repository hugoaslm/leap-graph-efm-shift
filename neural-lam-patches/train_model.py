...
def main():
    ...
    parser.add_argument(
        ...
        "If provided, loads config via constants.load_experiment_config() "
    ...
    # Load experiment config if provided (must happen BEFORE dataset creation)
    ...
    assert args.eval in valid_eval_splits, (
        f"Unknown eval setting: {args.eval}; valid: {valid_eval_splits}"
    )
    ...
    # Load data
    if cfg is not None or args.dataset.startswith("global"):
